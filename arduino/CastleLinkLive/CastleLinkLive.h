...
/**
    This value can be used as "throttlePinNumber" argument to begin function
    to indicate that library has to generate throttle signal instead of
    reading it from an external source (i.e. a standard RC receiver)
    This is also the default value if you call begin without provinding 
    the second parameter.
    @see uint8_t CastleLinkLiveLib::begin(uint8_t nESC, int throttlePinNumber)
*/
...
typedef struct castle_esc_data_struct {
  ...
  float RPM;             /**< \brief Round per minutes the motor is spinning at.
  Keep in mind that this values represents ELECTRICAL rpm, that are not shaft/prop rpm. 
